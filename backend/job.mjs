	...
	async assign(req) {
		return new Promise((resolve, reject) => {
			...
			bus.on(`file`, (name, data, info) => {
				...
				data
					.pipe(WriteStream(`${this.dir}/${info.filename}`))
					.on(`error`, (err) => reject(err));
				this.files.push(info.filename);
			});
			bus.on(`field`, (name, data) => {
				form.set(name, data);
			});
			...
			bus.on(`close`, async () => {
				...
				this.title = (command.title != `` ? command.title : genTitle()).replace(
					/\s/g,
					`_`
				);
				...
				resolve({
					...this,
					...(await this.application(command, this).catch((err) => {
						logger.error(err);
						this.status = `errored`;
					})),
				});
