...
const toolData = (path, script) => {
	return { path: path, script: script };
};

export const blast = async (
	{ type, db, outfmt = 10, args },
	{ title, dir, files }
) => {
	...
	return toolData(
		...
		files.map((file) => {
			return normalize(
				`${base} -query ${dir}/${file} -out ${path}/${file.replace(
					extname(file),
					ext
				)}`
			);
...
export const blastDb = async ({ type, args }, { title, dir, files }) => {
	...
	const makeblastdb = (file) => {
		return normalize(
			`makeblastdb -in ${dir}/${file} -dbtype ${type} -title ${title} -out ${path}/${title} ${args ?? ``
			}`
		);
	};
	return toolData(
		path,
		files.length > 1 ? await concatDb() : makeblastdb(files)
	);
};
...
export const rodeo = async ({ args }, { title, dir, files }) => {
	...
	return toolData(
		...
		files.map((file) => {
			return normalize(
				`rodeo2 ${dir}/${file} -hmm /hmm_dir/ -out ${path} ${args ?? ``}`
			);
		})
	);
};

export const prodigal = async (
	{ format = `gbk`, args },
	{ title, dir, files }
) => {
	...
	return toolData(
		...
		files.map((file) => {
			return normalize(
				`prodigal -i ${dir}/${file} -f ${format} -a ${path}/${file.replace(
					extname(file),
					`.${format}`
				)} ${args ?? ``}`
			);
...
export const skaniDist = async ({ db, args }, { title, dir, files }) => {
	...
	return toolData(
		...
		files.map((file) => {
			return normalize(
				`skani dist -r /shared/skaniSketch/${db}/sketch/* -q ${dir}/${file} -t 1 -o ${path}/${file} ${args ?? ``
				}`
			);
		})
	);
...
export const skaniSearch = async ({ db, args }, { title, dir }) => {
	...
	return toolData(
		path,
		normalize(
			`skani search -d /shared/skaniSketch/${db}/sketch/ -o ${path}/${title}.tsv -q ${dir}/* -t 1 ${args ?? ``
			}`
		)
	);
};
...
export const skaniSketch = async ({ args }, { title, dir }) => {
	...
	return toolData(
		path,
		normalize(`skani sketch ${dir}/* -o ${path}/sketch/ -t 1 ${args ?? ``}`)
	);
};
...
export const skaniTriangle = async ({ args }, { title, dir }) => {
	...
	return toolData(
		path,
		normalize(
			`skani triangle ${dir}/* -o ${path}/${title}.tsv -t 1 ${args ?? ``}`
		)
	);
};
