...
const deleteOne = async(collection, id) => {
	return (await database.collection(collection).deleteOne({"_id":id})).deletedCount
}
...
const genDbRoutes = (collections) => {
	collections.map(collection => {
		...
		router.delete(`/${URL}/:id`, async (req, res) => {
			...
			params.dir?rm(params.dir, { recursive: true }, async (err) => res.sendStatus(await deleteOne(collection, req.params.id)?200:404) )
			:res.sendStatus(await deleteOne(collection, req.params.id)?200:404)
		})
	})
}
