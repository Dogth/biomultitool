import {stat, readdir, rm, statSync, mkdir, ReadStream, WriteStream} from 'fs'
...
const sendArhive = (dir, res) => {
	...
	archive.finalize()
	archive.on('error', (err) => { logger.error(err); res.sendStatus(500) })
...
const sendFile = (dir, res) => {
	ReadStream(dir)
	.on(`error`, (err) => { logger.error(err); res.sendStatus(500) })
	.pipe(res)
}
...
export const download = (dir, res) => {
	...
	stat(path, (err, file) => {
		err?res.sendStatus(400):file.isDirectory()?
		sendArhive(path, res):sendFile(path, res)
	})
}
...
export const upload = (req, res) => {
	...
	try{
		...
		bus.on(`file`, (name, data, info) =>{ 
			mkdir(path, { recursive:true }, (err => err?
				res.sendStatus(500)
				:data.pipe(WriteStream(`${path}/${info.filename.replace(/\s/g, `_`)}`)))) })
				.on(`error`, (err) => { res.sendStatus(err?500:200); console.log(err) })
			.on(`field`, (name, data) => { console.log(`Field ${data}`); Readable.from(data).pipe(WriteStream(path)) })
			.on(`error`, (err) => { res.sendStatus(500); logger.error(err) })
...
export const explore = (dir, res) => {
	const path = normalize(`/shared/${dir}`)
		stat(path, (err, file) => { err?res.sendStatus(404):file.isDirectory()?
