...
export const getData = async (file, hook, ref) => {
	if (file.size > 1000000)
		ref.current = { data: `File too large to preview`, info: file };
	else
		ref.current = {
			data: (await API.get(`/files${file.path}`)).data,
			info: file,
		};
	hook(ref.current);
};
...
export const sendForm = (data, path, hook, onDone) => {
	API.post(`${path}`, data, {
		...
		onUploadProgress: (ProgressEvent) => {
			hook(Math.floor((ProgressEvent.loaded * 100) / ProgressEvent.total));
		},
	})
		.then((res) => onDone(res))
...
export const DownloadLink = ({ path, filename }) => {
	return (
		<Link
			to={`${URL}/download${path}`}
			className="Download"
			target="_blank"
			download={filename}
		/>
