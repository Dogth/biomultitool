...
const set = {
	...
	get(`/db/tasks`).then(tasks => tasks.forEach(task => { set[task.status](prevState => [...prevState,  <Task key={ task._id } data={ task } />]) }))
}
...
return (
	<div className='Form'>
	<div className='DbWrapper'><div className='DbTitle'>Completed</div><div className='TaskList'>{ completed }</div></div>
	<div className='DbWrapper'><div className='DbTitle'>Running</div><div className='TaskList'>{ running }</div></div>
	<div className='DbWrapper'><div className='DbTitle'>Queued</div><div className='TaskList'>{ queued }</div></div>
	</div>
	)
}
