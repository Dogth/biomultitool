	...
	const getMany = (collection, hook) => {
		get(`/db/${collection}`).then((entries) => {
			hook(
				entries.map((entry) => {
					return (
						<DbEntry key={entry._id} data={entry} collection={collection} />
					);
				})
			);
		});
	...
	const DbEntry = ({ data, collection }) => {
		...
		return (
			<div
				className="Task"
				style={{
					background: dbBackgrounds[data.type] ?? `var(--paleOrange)`,
					...style,
				}}
			>
				<button
					className="Delete"
					onClick={() =>
						del(`/db/${collection}/${data._id}`, () => setStyle(deleteAnim))
					}
				></button>
	...
	return (
		<div className="DatabaseList">
			<div className="DbWrapper">
				...
				<div className="EntryList">{blastDb}</div>
			</div>
			<div className="DbWrapper">
				<div className="DbTitle">Skani</div>
				<div className="EntryList">{skaniSketch}</div>
			</div>
		</div>
	);
};
