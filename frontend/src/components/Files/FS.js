			...
			return (
				<div className="File" style={style}>
					<button
						className="Delete"
						onClick={() => {
							del(`/files${data.path}`, setStyle(deleteAnimCard));
						}}
					/>
			...
			return (
				<div className="Info">
					...
					<p className="CreatedAt">
						{Intl.DateTimeFormat().format(new Date(data.createdAt))}
					</p>
					<p className="Size">
						{Intl.NumberFormat({ style: `unit`, unit: `byte` }).format(
							data.size
						)}
						B
					</p>
				...
				<div className="New">
					<form
						onChange={handleSubmit((data) =>
							sendForm(data, path, setProgress, getFiles)
						)}
					>
			...
			const types = {
				"text/html": (
					<iframe
						...
						src={`data:${data.info.mime};base64,${binTo64(data.data)}`}
					/>
				),
				"image/svg+xml": (
					<img
			...
			return (
				<div className="Preview">
						...
						{data.info.path ? (
							<>
								<button
									className="Delete"
									onClick={() =>
										del(`/files${data.info.path}`, () => {
											setPreview(previewInfo);
											getFiles();
										})
									}
								/>
								<DownloadLink path={data.info.path} />
							</>
						) : null}
					...
					<div className="FileData">
						{types[data.info.mime] ?? <code>{data.data}</code>}
					</div>
				</div>
			);
		};
